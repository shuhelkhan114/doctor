...
const UpdateAddressScreen: React.FC<UpdateAddressScreenProps> = (props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit(values) {
      updateProfile({
        ...
        ...(values.apartment.trim() === '' ? { apartment: null } : { apartment: values.apartment }),
        city: values.city,
        state: values.state,
        zip: values.zip,
      });
  ...
  useEffect(() => {
    if (auth.doctor) {
      formik.setValues({
        address: auth.doctor.address,
        apartment: auth.doctor.apartment,
        city: auth.doctor.city,
        state: auth.doctor.state,
        zip: auth.doctor.zip,
      });
    }
  }, []);
